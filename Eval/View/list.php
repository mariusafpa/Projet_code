...
<table class="table table-bordered border-dark mt-3">
    <thead>
        ...
        <th scope="col" class="table-secondary text-center">Libell&eacute;</th>
...
<?php
    ...
    require "../Models/connexion_bdd.php"; // Connexion base de données
    $requete = "SELECT * FROM produits";
    $result = $db->query($requete);
    $nbLigne = $result->rowCount(); // Nombre de lignes retournées par la requête


    $rows = $result->fetchAll(PDO::FETCH_CLASS,'Controler\Controler');

        foreach ($rows as $row):// Tant qu'il y a des lignes à afficher :
        ?>

        <tr>
        <td class="table-warning text-center"><img src="<?php echo $row->getImg()?>"  width="150px"></td></td>
        <td class="table-secondary text-center"><?= $row->pro_id; ?></td>
            ...
            <td class="table-secondary text-center">
                <?php 
                    if($row->pro_bloque==1){
                        echo '<span class="badge badge-danger">Bloqué</span>';
                    }else echo'';
                ?>
            </td>
...
<a href="index.php"><button type="button" class="btn btn-secondary btn-lg">Retour Index</button></a>
<a href="ajout.php"><button type="button" class="btn btn-success btn-lg">Ajout</button></a>
</div>
<br>
<footer>
